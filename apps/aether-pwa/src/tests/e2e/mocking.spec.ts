...
test.describe('Mocked Data & UI State', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      (window as Window & { __USE_REAL_FETCH__?: boolean }).__USE_REAL_FETCH__ = true;
    });
  ...
  test('should transition Orb state on session start', async ({ page, isMobile }) => {
    ...
    const statusMessage = page
      .getByText('Connecting...', { exact: false })
    ...
    const orbContainer = page
      .getByTestId('orb-container')
      .or(page.locator('.orb-container'))
      .or(page.getByLabel('Start Session'));
