...
interface UseRealtimeSessionProps {
  ...
  onLimitReached: () => void;
  // Allow passing a pre-created/resumed context
  providedAudioContext?: AudioContext | null;
  // Callback when permission is denied/error
  onPermissionError?: (error: Error) => void;
}

// VAD Constants - defaults that can be overridden by backend config
// TODO: Backend should send these in session init response
const VAD_THRESHOLD = 0.01; // RMS Threshold (Lowered for sensitivity)
const SILENCE_DURATION = 500; // ms (Reduced for snappier response - Doubao-inspired)
const AUDIO_TIMEOUT_MS = 3000; // 3s timeout (Increased for slower networks)

// Map our language codes to Web Speech API compatible codes
const SPEECH_LANG_MAP: Record<string, string> = {
  auto: 'yue-Hant-HK', // Default to Cantonese for auto-detect
  'toi-HK': 'yue-Hant-HK', // Toishanese → Cantonese (no native Web Speech API support)
  'yue-HK': 'yue-Hant-HK', // Cantonese (Hong Kong)
  'cmn-CN': 'cmn-Hans-CN', // Mandarin (Simplified)
...
export function useRealtimeSession({
  ...
  const pendingAudioResolversRef = useRef<
    Array<(val: { data: Float32Array; sampleRate: number }) => void>
  >([]);
  ...
  useEffect(() => {
    ...
    return () => {
      stopMicrophone(); // Don't close provided context here, handle carefully
      stopSpeechRecognition();
    };
  }, [status, language]);
  ...
  const startSpeechRecognition = () => {
    try {
      ...
      recognition.lang = SPEECH_LANG_MAP[language] || 'yue-Hant-HK';
      ...
      recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        if (
          ...
          turnStartIndexRef.current >= event.results.length
        ) {
      ...
      recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
        ...
        if (event.error === 'not-allowed' && onPermissionError) {
          onPermissionError(new Error('Speech recognition not allowed'));
        }
      };
  ...
  const startMicrophone = async () => {
    try {
      // 1. Get User Media
      // Note: We removed strict sampleRate: 16000 constraint for Safari compatibility.
      // We will let the AudioContext handle the sample rate or resampling if needed.
      const stream = await navigator.mediaDevices.getUserMedia({
        audio: {
          echoCancellation: true,
          noiseSuppression: true,
          autoGainControl: true,
          // sampleRate: 16000 // Removed to avoid OverconstrainedError on some devices
        },
      });
      ...
      // Setup Audio Context
      let ctx = providedAudioContext;
      if (!ctx) {
        // Fallback: Create new context if not provided
        // Try to match 16k, but fallback to default if not supported
        try {
      ...
      processor.onaudioprocess = (e) => {
        ...
        const currentState = currentVoiceStateRef.current;

        // BARGE-IN SUPPORT: Allow listening during 'speaking' state for interrupts
        // Only mute during 'idle' and 'processing' states
        ...
        // 2. BARGE-IN: If speaking (TTS playing) and user starts talking, interrupt
        let effectiveState = currentState;
        if (currentState === 'speaking' && rms > VAD_THRESHOLD && !isSpeakingRef.current) {
          logger.info('VAD', 'Barge-in detected - interrupting TTS');
          audioPlayer.stop(); // Stop TTS immediately
          isSpeakingRef.current = true;
          currentVoiceStateRef.current = 'listening'; // CRITICAL: Update ref immediately
          effectiveState = 'listening'; // Use updated state for rest of this frame

          // Clear text for new turn
          setTranscript('');
          setActiveText('');
          setActiveTextSource('user');
        ...
        // 3. Normal VAD Logic (only during 'listening' state)
      ...
      if (onPermissionError && e instanceof Error) {
        onPermissionError(e);
      }
    }
  };
        ...
        try {
          ...
          switch (msg.type) {
            case 'user.started_speaking':
              // CRITICAL: Stop any current playback immediately (Interruptibility)
              audioPlayer.stop();
            ...
            case 'ai.text':
              ...
              logger.debug(
                'WS',
                `Received ai.text: "${msg.text}". Paired with audio? ${!!audioPromise}`
              );
              ...
              playOperationChain.current = playOperationChain.current.then(async () => {
                if (audioPromise) {
                  try {
                    ...
                    if (data.length > 0) {
                      ...
                      const playbackPromise = audioPlayer.play(data, sampleRate, {
                        onStart: (actualDuration) => {
                          logger.debug(
                            'WS',
                            `Playback Started. Displaying text: "${msg.text}", duration: ${actualDuration.toFixed(2)}s`
                          );
                          if (msg.text) {
                    ...
                    logger.warn(
                      'WS',
                      `Audio error: ${error instanceof Error ? error.message : 'Unknown'}. Displaying text without audio sync.`
                    );
                    if (msg.text) {
                      setCurrentAssistantMessage(msg.text);
                      setActiveText(msg.text);
