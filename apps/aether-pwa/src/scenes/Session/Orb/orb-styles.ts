...
// Neural effect intensity configuration per voice state
export interface NeuralIntensityConfig {
  particleSpeed: number; // Multiplier for orbit speed (1 = base)
  particleOpacity: number; // 0-1
  particleCount: number; // Number of visible particles
  pulseSpeed: number; // Duration in seconds
  pulseScale: number; // Max scale for pulse glow
  ringSpeed: number; // Multiplier for ring rotation
  ringOpacity: number; // 0-1
  glowIntensity: number; // 0-1 for core glow
}

export const NEURAL_INTENSITY: Record<UIVoiceState, NeuralIntensityConfig> = {
  idle: {
    particleSpeed: 0.5,
    particleOpacity: 0.3,
    particleCount: 6,
