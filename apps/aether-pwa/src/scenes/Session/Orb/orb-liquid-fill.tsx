...
export const OrbLiquidFill = ({ state, emotionalTone = 'calm' }: OrbLiquidFillProps) => {
  ...
  return (
    <div
      className={`absolute inset-0 rounded-full bg-gradient-to-br ${gradientClass} ${shadowClass} duration-emphasis transition-all`}
    >
      {/* Internal highlighting for liquid effect */}
      <div className="absolute inset-0 rounded-full bg-gradient-to-t from-black/10 to-transparent opacity-50" />
    </div>
  );
};
