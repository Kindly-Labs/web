...
describe('AudioPlayer', () => {
  ...
  it('should queue audio if playing', async () => {
    ...
    await waitFor(() => {
      // Expect 2 calls: 1 for iOS unlock (in resume), 1 for actual playback
    ...
    // Second should NOT start yet (count remains 2 from the first play)
    expect(mockContext.createBufferSource).toHaveBeenCalledTimes(2);
    ...
    await waitFor(() => {
      // Expect 4 calls total: 2 from first, 2 from second (resume + play)
