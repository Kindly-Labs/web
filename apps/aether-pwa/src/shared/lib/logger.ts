...
class AetherLogger {
  ...
  private getBrowserStyles(level: LogLevel) {
    ...
    switch (level) {
      case 'info':
        return `${base} background: #3b82f6; color: white;`; // Blue
      case 'warn':
        return `${base} background: #f59e0b; color: black;`; // Amber
      case 'error':
        return `${base} background: #ef4444; color: white;`; // Red
      case 'debug':
        return `${base} background: #6366f1; color: white;`; // Indigo
      default:
        return `${base} background: #6b7280; color: white;`; // Gray
    }
  }
  ...
  public error(category: string, message: string, data?: unknown, stack?: string) {
    this.log('error', category, message, data, stack);
  }
